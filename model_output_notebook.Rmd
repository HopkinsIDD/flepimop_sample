...
params:
  ...
  config: config_sample_2pop_ic_FF.yml #name of configuration file
  ...
  #seir_modifier_scenario:  # name of the scenario to plot results for. Required if multiple scenarios in config. Comment out if no scecnarios in config
  #outcomes_modifier_scenario: # name of the scenario to plot results for. Required if multiple scenarios in config. Comment out if no scecnarios in config
...
if (params$report_location == "offline"){
  ...
  install.packages(list.files("../flepimop/flepimop/R_packages/",full.names=TRUE),type='source',repos=NULL)
...
res_dir <- paste0(ifelse(is.null(opt$results_path),"",paste0(opt$results_path,"/")), 'model_output')
print(res_dir)
...
setup_prefix <- paste0(config$name,ifelse(is.null(config$seir_modifiers$scenarios),"",paste0("/",opt$seir_modifier_scenario)),ifelse(is.null(config$outcome_modifiers$scenarios),"",paste0("/",opt$outcome_modifier_scenario)))
#print(setup_prefix)
...
import_model_outputs <- function(scn_run_dir, inference, outcome, global_opt = NULL, final_opt = NULL, 
                                 lim_hosp = c("date", "incidC", "incidH", "incidD",config$subpop_setup$nodenames) # only used if outcome = 'hosp'
  ...
  subdir <- paste0(scn_run_dir, inference_filepath_suffix, "/",outcome)
  print(subdir)
...
```{r snpi, cache = TRUE, fig.dim = c(10,20), results='hide',fig.keep='all',eval = eval_snpi}
# read in model outputs
...
```{r hosp_daily_single_slot, results='asis', cache = TRUE, fig.dim = c(10,10), eval = eval_hosp}
...
## Quantiles {.tabset}
```{r hosp_daily_quantiles, results='asis', cache = TRUE, fig.dim = c(10,10), eval = eval_hosp}
...
```{r hnpi, cache = TRUE, fig.dim = c(10,20), results='hide',fig.keep='all', eval = eval_hnpi}
# read in model outputs
...
```{r hosp_trajectories_inference_aggregate, fig.dim = c(10,10), results='asis', eval = inference}
...
```{r hosp_aggregate_quantiles, fig.dim = c(10,10), results='asis', eval = inference}
...
```{r hosp_trajectories_by_likelihood, fig.dim = c(10,20), results='hide',fig.keep='all', eval=inference}
